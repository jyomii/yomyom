		...
		$(".calendarDiv").on(
				...
				function(event) {
					if ($(this).text() != "" || $(this).text().length() > 2) {
						...
						$('input[id=groupin_date]').val(date);
					}
				})
		$("input:checkbox[id='moneyCheck']").click(function() {
	...
	function buildCalendar() {//현재 달 달력 만들기
		...
		for (i = 1; i <= lastDate.getDate(); i++) {
			...
			if (cnt % 7 == 1) {/*일요일 계산*/
				...
				cell.innerHTML = "<input type='hidden' value='일'><a href='#' class='caldendarCell'><font color=#F79DC2>"
						+ i + "</a>";
				//1번째의 cell에만 색칠
			}
			if (cnt % 7 == 2) {/*일요일 계산*/
				//1주일이 7일 이므로 일요일 구하기
				//월화수목금토일을 7로 나눴을때 나머지가 1이면 cnt가 1번째에 위치함을 의미한다
				cell.innerHTML = "<input type='hidden' value='월'><a href='#' class='caldendarCell'>"
						+ i + "</a>";
				//1번째의 cell에만 색칠
			}
			if (cnt % 7 == 3) {/*일요일 계산*/
				//1주일이 7일 이므로 일요일 구하기
				//월화수목금토일을 7로 나눴을때 나머지가 1이면 cnt가 1번째에 위치함을 의미한다
				cell.innerHTML = "<input type='hidden' value='화'><a href='#' class='caldendarCell'>"
						+ i + "</a>";
				//1번째의 cell에만 색칠
			}
			if (cnt % 7 == 4) {/*일요일 계산*/
				//1주일이 7일 이므로 일요일 구하기
				//월화수목금토일을 7로 나눴을때 나머지가 1이면 cnt가 1번째에 위치함을 의미한다
				cell.innerHTML = "<input type='hidden' value='수'><a href='#' class='caldendarCell'>"
						+ i + "</a>";
				//1번째의 cell에만 색칠
			}
			if (cnt % 7 == 5) {/*일요일 계산*/
				//1주일이 7일 이므로 일요일 구하기
				//월화수목금토일을 7로 나눴을때 나머지가 1이면 cnt가 1번째에 위치함을 의미한다
				cell.innerHTML = "<input type='hidden' value='목'><a href='#' class='caldendarCell'>"
						+ i + "</a>";
				//1번째의 cell에만 색칠
			}
			if (cnt % 7 == 6) {/*일요일 계산*/
				//1주일이 7일 이므로 일요일 구하기
				//월화수목금토일을 7로 나눴을때 나머지가 1이면 cnt가 1번째에 위치함을 의미한다
				cell.innerHTML = "<input type='hidden' value='금'><a href='#' class='caldendarCell'>"
						+ i + "</a>";
				//1번째의 cell에만 색칠
			}
			if (cnt % 7 == 0) {/* 1주일이 7일 이므로 토요일 구하기*/
				//월화수목금토일을 7로 나눴을때 나머지가 0이면 cnt가 7번째에 위치함을 의미한다
				cell.innerHTML = "<input type='hidden' value='토'><a href='#' class='caldendarCell'><font color=skyblue>"
						+ i + "</a>";
				//7번째의 cell에만 색칠
										...
										<div class="editing-info">
											...
											<div class="form-radio">
												<div class="radio">
													<label> <input type="radio" checked="checked"
														name="radio"><i class="check-box"></i>신청순
													</label>
												</div>
												<div class="radio">
													<label> <input type="radio" name="radio"><i
														class="check-box"></i>입금순
													</label>
												</div>
											</div>
											...
											<form method="post">
												<div class="form-group">
													<input type="text" required="required" id="groupin_name" />
													<label class="control-label" for="input">정모 제목</label><i
														class="mtrl-select"></i>
												</div>
												<div class="form-group half">
													<input type="text" id="groupin_date" required="required" />
												...
												<div class="form-group half halfmargin">
													<input type="text" required="required" /> <label
														class="control-label" for="input">장소</label><i
														class="mtrl-select"></i>
												</div>
												<div class="form-group">
												...
												<div id="moneyDiv" class="form-group half moneyDiv">
													<input type="text" id="moneyText" required="required" /> <label
														class="control-label" for="input">회비</label><i
														class="mtrl-select"></i>
												</div>

												<div class="submit-btns">
													<button type="button" class="mtr-btn btn-center">
														<span>정모추가</span>
													</button>
