	...
	<ul class="naves">
		<c:forEach var="gbl" items="${groupboardlist }">
			<li>
				...
				<input type="hidden" value="${gbl.boardType }"> 
				<a href="groupin_group_board_transfer.net?groupkey=${group.groupKey }&boardkey=${gbl.boardKey }&boardtype=${gbl.boardType}&boardname=${gbl.boardName}" title="">${gbl.boardName}</a> 
...
<c:if test="${userinfo.userGrade==1}">
	<div class="widget">
		...
		<ul class="naves">
			<li><i class="ti-info-alt"></i> <a href="groupin_group_admin.net?groupkey=${group.groupKey}" title="">모임 기본 설정</a></li>
			<li><i class="ti-mouse-alt"></i> <a href="groupin_group_admin_scheduleList.net?groupkey=${group.groupKey}" title="">모임 일정 관리</a></li>
			<li><i class="ti-heart"></i> <a href="groupMember?groupkey=${group.groupKey}" title="">모임 회원 관리</a></li>
			<li><i class="ti-settings"></i> <a href="groupin_group_admin_board.net?groupkey=${group.groupKey}" title="">모임 게시판 관리</a></li>
			<li><i class="fas fa-user-plus"></i><a href = "group_admin_signup.net?groupkey=${group.groupKey}">모임 가입 설정</a></li>
		</ul>
	</div>
</c:if>
<!-- 모임 관리 게시판 위젯 -->
...
<c:if test="${not empty group.groupkatalk }">
	<div class="widget">
		<div class="banner medium-opacity">
			...
			<div style="background-image: url(resources/images/katalk.jpg)"
				class="bg-image"></div>
			<!--  <div class="baner-top">
